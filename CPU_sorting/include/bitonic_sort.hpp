...
inline std::size_t add_extra_elem(ContainerT& cont,
...
inline void bitonic_sort(ContainerT& cont, CompareT comp)
{
    ...
    const std::size_t extra_elems =
        detail::add_extra_elem(cont, num_elems, comp);
    const std::size_t ext_num_elems = num_elems + extra_elems;
    auto first = cont.begin();
    auto last = cont.end();

    for (std::size_t merge_size = 2; merge_size <= ext_num_elems;
         merge_size *= 2) {
        bool ascending_block = true;
        for (auto it = first; it != last; it += merge_size) {
            for (std::size_t split_size = merge_size / 2; split_size >= 1;
                 split_size /= 2) {
                for (std::size_t block = 0; block < merge_size;
                     block += 2 * split_size) {
